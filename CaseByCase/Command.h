...
public:
	...
	void checkCommand(const string& s)
	{
		...
		else if (tokens[0] == "/removelog") cmd_removelog();
	...
	void cmd_help()
	{
		...
		canvas->input("/save				저장하기");
		canvas->input("/load				불러오기");
		canvas->input("/help				명령어 리스트 출력");
		canvas->input("/clear				cmd창 지우기");
		//canvas->input("/addtxt [이름] [내용]		txt파일 생성");
		//canvas->input("/addexe [이름] [코드]		exe파일 생성");
		//canvas->input("/addfolder [이름]			폴더파일 생성");
		//canvas->input("/remove [이름]			대상파일 제거");

		canvas->input("/scan				주변 IP 스캔");
		//canvas->input("/portscan				포트 정보 확인");
		canvas->input("/target [IP]				주변 IP 스캔");
		canvas->input("/crack [Port]			지정한 포트 열기");
		canvas->input("/nuke (IP)					PC 해킹");
		canvas->input("/connect	(IP)			해당 컴퓨터 접속");
		//canvas->input("/removelog				로그 삭제"); ...개발중
		canvas->input("/disconnect			접속 종료");

		canvas->input("/in [파일] (비밀번호)			파일 열기");
		canvas->input("/out				이전으로 돌아감 (상위폴더로 나가기)");
		//canvas->input("/decoding [파일]			대상 파일 복호화"); ...개발중
		canvas->input("--------------------------------------------------------");
	}
